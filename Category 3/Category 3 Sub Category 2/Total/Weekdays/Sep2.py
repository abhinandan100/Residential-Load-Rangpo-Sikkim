#One-Room Minimum Connected Load
#Create new user classes
OR = User("One room minimal",1,1)
User_list.append(OR)

#One-Room Minimum Connected Load
OR_indoor_Tubelight1 = OR.Appliance(OR,1,55,2,120,0.15,15)
OR_indoor_Tubelight1.windows([1290,1440],[0,120],0.1)
OR_indoor_Tubelight2 = OR.Appliance(OR,1,55,2,120,0.15,15, occasional_use = 0.5)
OR_indoor_Tubelight2.windows([1290,1440],[0,120],0.3)
OR_indoor_Tubelight3 = OR.Appliance(OR,1,55,2,120,0.15,1)
OR_indoor_Tubelight3.windows([1290,1440],[0,120],0.1)
OR_indoor_Tubelight4 = OR.Appliance(OR,1,55,2,120,0.15,15, occasional_use = 0.5)
OR_indoor_Tubelight4.windows([1290,1440],[0,120],0.3)
OR_Fan = OR.Appliance(OR,2,70, 2, 720,0.15)
OR_Fan.windows([1290,1440],[0,840],0.2)
OR_Fan1 = OR.Appliance(OR,1,70, 2, 360,0.15,15,occasional_use=0.5)
OR_Fan1.windows([1290,1440],[0,840],0.2)
OR_lavatory_LEDBulb = OR.Appliance(OR,1,5,2,60,0.2,15)
OR_lavatory_LEDBulb.windows([1290,1440],[300,840],0.1,[0,299])
OR_Phone_charger = OR.Appliance(OR,4,5,2,120,0.3,15)
OR_Phone_charger.windows([1290,1440],[420,840],0.35)
OR_TV=OR.Appliance(OR,1,60,2,300,0.2,30)
OR_TV.windows([1290,1440],[480,840],0.15)
OR_SetTopBox = OR.Appliance(OR,1,10,2,300,0.2,30)
OR_SetTopBox.windows([1290,1440],[480,840],0.15)
OR_KitchenTubelight=OR.Appliance(OR,1,40,2,180,0.15,15)
OR_KitchenTubelight.windows([1320,1440],[0,120],0.2)
OR_KitchenExhaustFan=OR.Appliance(OR,1,40,3,180,0.15,15)
OR_KitchenExhaustFan.windows([1320,1440],[480,540],0.2,[720,840])
OR_balcony_LED = OR.Appliance(OR,1,9,2,30,0.15,5)
OR_balcony_LED.windows([1320,1440],[0,0],0.1)
OR_Fridge = OR.Appliance(OR,1,200,1,1440,0,45, 'yes',1)
OR_Fridge.windows([0,1440],[0,0])
OR_Fridge.specific_cycle_1(5,45,200,45)
OR_Fridge.cycle_behaviour([0,1440],[0,0])
OR_MixerGrinder = OR.Appliance(OR,1,500,2,60,0.15,15,occasional_use = 0.33)
OR_MixerGrinder.windows([1320,1440],[480,840],0.3)
OR_InductionCooktop = OR.Appliance(OR,2,2000,3,150,0.1,15, 'yes',1)
OR_InductionCooktop.windows([1320,1440],[480,540],0.15, [720,840])
OR_InductionCooktop.specific_cycle_1(500,15,2000,15)
OR_InductionCooktop.cycle_behaviour([1320,1440],[720,840])
OR_WaterPurifier = OR.Appliance(OR,1,50,2,60,0.15,15)
OR_WaterPurifier.windows([420,780],[0,0],0.3)
OR_Iron = OR.Appliance(OR,1,1200,2,30,0.15,15, occasional_use=0.5)
OR_Iron.windows([360,780],[0,0],0.3)
OR_laptop = OR.Appliance(OR,1,50,2,60,0.15,30,occasional_use = 0.33)
OR_laptop.windows([480,840],[0,0],0.1)
OR_hairtrimmercharger = OR.Appliance(OR,1,5,2,30,0.15,15,occasional_use = 0.2)
OR_hairtrimmercharger.windows([480,780],[0,0],0.1)
OR_WashingMachine=OR.Appliance(OR,1,600,2,30,0.2,30, occasional_use=0.5)
OR_WashingMachine.windows([480,780],[0,0], 0.15)
OR_VacuumCleaner=OR.Appliance(OR,1,1000,2,30,0.2,30, occasional_use=0.5)
OR_VacuumCleaner.windows([420,600],[0,0], 0.15)
OR_MicroWaveOven=OR.Appliance(OR,1,1200,2,60,0.2,15, occasional_use=0.5)
OR_MicroWaveOven.windows([480,780],[0,0], 0.15)
OR_MusicSystem = OR.Appliance(OR,1,200,1,60,0.15,15, occasional_use=0.25)
OR_MusicSystem.windows([480,840],[0,0],0.3)
OR_BathroomGeyser = OR.Appliance(OR,1,2000,2,60,0.1,15, 'yes',1, occasional_use=.5)
OR_BathroomGeyser.windows([480,780],[1290,1350],0.1)
OR_BathroomGeyser.specific_cycle_1(2000,10,10,5)
OR_BathroomGeyser.cycle_behaviour([480,780],[1290,1350])
OR_MosquitoRepellent = OR.Appliance(OR,2,7,2,480,0.15,30)
OR_MosquitoRepellent.windows([480,780],[1290,1350],0.35)




#One-Room Minimum Connected Load
#Create new user classes
OR1 = User("One room minimal",3,1)
User_list.append(OR1)

#One-Room Minimum Connected Load
OR1_indoor_Tubelight1 = OR1.Appliance(OR1,1,55,2,240,0.15,15)
OR1_indoor_Tubelight1.windows([1290,1440],[0,120],0.1)
OR1_indoor_Tubelight2 = OR1.Appliance(OR1,1,55,2,120,0.15,15, occasional_use = 0.5)
OR1_indoor_Tubelight2.windows([1290,1440],[0,120],0.3)
OR1_indoor_Tubelight3 = OR1.Appliance(OR1,1,55,2,240,0.15,1)
OR1_indoor_Tubelight3.windows([1290,1440],[0,120],0.1)
OR1_indoor_Tubelight4 = OR1.Appliance(OR1,1,55,2,120,0.15,15, occasional_use = 0.5)
OR1_indoor_Tubelight4.windows([1290,1440],[0,120],0.3)
OR1_Fan = OR1.Appliance(OR1,2,70, 2, 720,0.15)
OR1_Fan.windows([1290,1440],[0,840],0.2)
OR1_Fan1 = OR1.Appliance(OR1,1,70, 2, 360,0.15,15,occasional_use=0.5)
OR1_Fan1.windows([1290,1440],[0,840],0.2)
OR1_lavatory_LEDBulb = OR1.Appliance(OR1,1,5,2,60,0.2,15)
OR1_lavatory_LEDBulb.windows([1290,1440],[300,840],0.1,[0,299])
OR1_Phone_charger = OR1.Appliance(OR1,4,5,2,120,0.3,15)
OR1_Phone_charger.windows([1290,1440],[420,840],0.35)
OR1_TV=OR1.Appliance(OR1,1,60,2,300,0.2,30)
OR1_TV.windows([1290,1440],[480,840], 0.15)
OR1_SetTopBox = OR1.Appliance(OR1,1,10,2,300,0.2,30)
OR1_SetTopBox.windows([1290,1440],[480,840],0.15)
OR1_KitchenTubelight=OR1.Appliance(OR1,1,40,2,180,0.15,15)
OR1_KitchenTubelight.windows([1320,1440],[0,120],0.2)
OR1_KitchenExhaustFan=OR1.Appliance(OR1,1,40,3,180,0.15,15)
OR1_KitchenExhaustFan.windows([1320,1440],[480,540],0.2,[720,840])
OR1_balcony_LED = OR1.Appliance(OR1,1,9,2,30,0.15,5)
OR1_balcony_LED.windows([1320,1440],[0,0],0.1)
OR1_Fridge = OR1.Appliance(OR1,1,200,1,1440,0,45, 'yes',1)
OR1_Fridge.windows([0,1440],[0,0])
OR1_Fridge.specific_cycle_1(5,45,200,45)
OR1_Fridge.cycle_behaviour([0,1440],[0,0])
OR1_MixerGrinder = OR1.Appliance(OR1,1,500,2,60,0.15,15,occasional_use = 0.33)
OR1_MixerGrinder.windows([1320,1440],[480,840],0.3)
OR1_WaterPurifier = OR1.Appliance(OR1,1,50,2,60,0.15,15)
OR1_WaterPurifier.windows([480,780],[0,0],0.3)
OR1_Iron = OR1.Appliance(OR1,1,1200,2,30,0.15,15, occasional_use=0.5)
OR1_Iron.windows([480,780],[0,0],0.3)
OR1_WashingMachine=OR1.Appliance(OR1,1,600,2,30,0.2,30, occasional_use=0.5)
OR1_WashingMachine.windows([480,780],[0,0], 0.15)
OR1_MicroWaveOven=OR1.Appliance(OR1,1,1200,2,60,0.2,15, occasional_use=0.5)
OR1_MicroWaveOven.windows([480,780],[0,0], 0.15)
OR1_MosquitoRepellent = OR1.Appliance(OR1,2,7,2,480,0.15,30)
OR1_MosquitoRepellent.windows([480,780],[1290,1350],0.35)


#One-Room Minimum Connected Load
#Create new user classes
OR2 = User("One room minimal",2,1)
User_list.append(OR2)

#One-Room Minimum Connected Load
OR2_indoor_LEDlight1 = OR2.Appliance(OR2,2,7,2,240,0.15,15)
OR2_indoor_LEDlight1.windows([1290,1440],[0,120],0.1)
OR2_indoor_LEDlight2 = OR2.Appliance(OR2,2,7,2,120,0.15,15, occasional_use = 0.5)
OR2_indoor_LEDlight2.windows([1290,1440],[0,120],0.3)
OR2_Fan = OR2.Appliance(OR2,1,70, 2, 720,0.15)
OR2_Fan.windows([1290,1440],[0,840],0.2)
OR2_Fan1 = OR2.Appliance(OR2,2,70, 2, 360,0.15,15,occasional_use=0.5)
OR2_Fan1.windows([1290,1440],[0,840],0.2)
OR2_lavatory_LEDBulb = OR2.Appliance(OR2,1,5,2,60,0.2,15)
OR2_lavatory_LEDBulb.windows([1290,1440],[300,840],0.1,[0,299])
OR2_Phone_charger = OR2.Appliance(OR2,4,5,2,120,0.3,15)
OR2_Phone_charger.windows([1290,1440],[420,840],0.35)
OR2_TV=OR2.Appliance(OR2,1,60,2,300,0.2,30)
OR2_TV.windows([1290,1440],[480,840], 0.15)
OR2_SetTopBox = OR2.Appliance(OR2,1,10,2,300,0.2,30)
OR2_SetTopBox.windows([1290,1440],[480,840],0.15)
OR2_KitchenLEDlight=OR2.Appliance(OR2,1,40,2,180,0.15,15)
OR2_KitchenLEDlight.windows([1320,1440],[0,120],0.2)
OR2_KitchenExhaustFan=OR2.Appliance(OR2,1,40,3,180,0.15,15)
OR2_KitchenExhaustFan.windows([1320,1440],[480,540],0.2,[720,840])
OR2_Fridge = OR2.Appliance(OR2,1,200,1,1440,0,45, 'yes',1)
OR2_Fridge.windows([0,1440],[0,0])
OR2_Fridge.specific_cycle_1(5,45,200,45)
OR2_Fridge.cycle_behaviour([0,1440],[0,0])
OR2_WaterPurifier = OR2.Appliance(OR2,1,50,2,60,0.15,15)
OR2_WaterPurifier.windows([480,780],[0,0],0.3)
OR2_MosquitoRepellent = OR2.Appliance(OR2,2,7,2,480,0.15,30)
OR2_MosquitoRepellent.windows([480,780],[1290,1350],0.35)

